...
function getAgentStatus() {
  ...
  for (const agent of agents) {
    try {
      ...
      let totalTokens = 0, tokenSources = 0, maxTokens = 0, minTokens = Infinity;
      if (fs.existsSync(sessionsPath)) {
          ...
          try {
            ...
            if (lastLine) {
              ...
              if (data.totalTokens) {
                ...
                maxTokens = Math.max(maxTokens, data.totalTokens);
                minTokens = Math.min(minTokens, data.totalTokens);
                tokenSources++;
              }
            }
          } catch (e) {}
      ...
      let statusText = '空闲';
      if (isActive) statusText = isHighLoad ? '高负载' : '忙碌';
      status[agent] = { 
      ...
      status[agent] = { name: `WALL-${agent.toUpperCase().replace('WALL-', '')}`, model: '?', emoji: '❓', sessions: 0, tokens: 0, tokenLimit: 200000, tokenPercent: 0, status: '离线' };
    }
  }
  return status;
}
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .status-light.highload { background: #ff5252; box-shadow: 0 0 12px #ff5252; }
...
<body>
  ...
  <script>
    ...
    function updateClock() {
      ...
      const nowFull = now.toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' });
      document.getElementById('refresh-info').textContent = '🔄 Last update: ' + nowFull + ' • Auto-refresh 10s';
    }
...
server.listen(PORT, () => {
  console.log(`🎯 WALL Team Monitor running at http://localhost:${PORT}/`);
